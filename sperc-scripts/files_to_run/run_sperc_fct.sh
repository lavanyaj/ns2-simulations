...
if [ "${TOPOLOGY_TYPE}" == "b4" ]; then
    ...
    declare -a versions=(
    # FCT in data center version, sperc basic, note we don't prioritize short flows (SPERC_PRIO_THRESH_INDEX=0, SPERC_PRIO_WORKLOAD set in sperc.tcl) and wait to get rate before start (INIT_SPERC_DATA_INTERVAL=-1). All flows have same weight of 1 (SPERC_WEIGHTED_MAX_MIN_NUM_CLASSES=1).
	"QUEUE_SIZE=83; MEAN_LINK_DELAY=0.0000002; HOST_DELAY=0.0000025; SPERC_CONTROL_TRAFFIC_PC=2; SPERC_HEADROOM_PC=2; SPERC_MAXSAT_TIMEOUT=0.000020; SPERC_MAXRTT_TIMEOUT=0.000100; SPERC_SYN_RETX_PERIOD_SECONDS=0.000040; SPERC_FIXED_RTT_ESTIMATE_AT_HOST=0.000012; SPERC_CONTROL_PKT_HDR_BYTES=64; SPERC_DATA_PKT_HDR_BYTES=40; SPERC_INIT_SPERC_DATA_INTERVAL=-1; MATCH_DEMAND_TO_FLOW_SIZE=1;  SPERC_PRIO_THRESH_INDEX=0; SPERC_WEIGHTED_MAX_MIN_NUM_CLASSES=1; SPERC_JITTER=0; SPERC_UPDATE_IN_REV=1; KW=\"wt1-pri0-match-ign\""    
    # FCT in data center version, sperc short, note we prioritize short flows (SPERC_PRIO_THRESH_INDEX=2, see *_thresh_for_prio array in sperc/sperc-host.cc to see that this is 145KB or 1BDP, SPERC_PRIO_WORKLOAD set in sperc.tcl) and start them at line-rate (INIT_SPERC_DATA_INTERVAL=0.00000012). All flows have same weight of 1 (SPERC_WEIGHTED_MAX_MIN_NUM_CLASSES=1).
    	"QUEUE_SIZE=83; MEAN_LINK_DELAY=0.0000002; HOST_DELAY=0.0000025; SPERC_CONTROL_TRAFFIC_PC=2; SPERC_HEADROOM_PC=2; SPERC_MAXSAT_TIMEOUT=0.000020; SPERC_MAXRTT_TIMEOUT=0.000100; SPERC_SYN_RETX_PERIOD_SECONDS=0.000040; SPERC_FIXED_RTT_ESTIMATE_AT_HOST=0.000012; SPERC_CONTROL_PKT_HDR_BYTES=64; SPERC_DATA_PKT_HDR_BYTES=40; SPERC_INIT_SPERC_DATA_INTERVAL=0.00000012; MATCH_DEMAND_TO_FLOW_SIZE=1; SPERC_PRIO_THRESH_INDEX=2; SPERC_WEIGHTED_MAX_MIN_NUM_CLASSES=1; SPERC_JITTER=0; SPERC_UPDATE_IN_REV=1; KW=\"wt1-pri2-match-start100-short-ign\""    
    ) ;
else
